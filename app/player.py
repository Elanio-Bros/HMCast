...
class Player:
    ...
    def start(self, input_file: str, output_dir: str, segments: list[tuple[float, float]], channel_type: str = "TV", start_number: int = 0):
        """
        ...
        # Calcular duração total para os fades
        total_duration = sum(dur for _, dur in segments) if segments else 0
        apply_fade = total_duration > 2.0
        ...
        f_parts = []
        v_label_in = "[0:v]"
        a_label_in = "[0:a:0]" # Alterado de 0:a:0? para mapeamento seguro com dynaudnorm
        ...
        if len(segments) == 1:
            ...
            if apply_fade and channel_type != "RADIO":
                f_parts.append(f"[0:v]fade=t=in:st=0:d=1,fade=t=out:st={total_duration - 1}:d=1[v_faded]")
                v_label_in = "[v_faded]"
                
            if apply_fade:
                f_parts.append(f"[0:a:0]afade=t=in:st=0:d=1,afade=t=out:st={total_duration - 1}:d=1,dynaudnorm[a_out]")
            else:
                f_parts.append(f"[0:a:0]dynaudnorm[a_out]")
            a_label_in = "[a_out]"
            ...
            if apply_fade and channel_type != "RADIO":
                f_parts.append(f"[v_full]fade=t=in:st=0:d=1,fade=t=out:st={total_duration - 1}:d=1[v_faded]")
                v_label_in = "[v_faded]"
            else:
                v_label_in = "[v_full]"
                
            if apply_fade:
                f_parts.append(f"[a_full]afade=t=in:st=0:d=1,afade=t=out:st={total_duration - 1}:d=1,dynaudnorm[a_out]")
            else:
                f_parts.append(f"[a_full]dynaudnorm[a_out]")
            a_label_in = "[a_out]"
        ...
        if channel_type == "RADIO":
            ...
            # Modo TV com Letterboxing (AspectRatio preservation)
            tv_filters = (
                f"{v_label_in}split=3[v1080_pre][v720_pre][v480_pre];"
                "[v1080_pre]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black[v1080out];"
                "[v720_pre]scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2:color=black[v720out];"
        ...
        if channel_type != "RADIO":
            cmd += ["-r", "30", "-fps_mode", "cfr"]

        cmd += [
            "-crf", "24" if channel_type == "RADIO" else "20",
            "-tune", "stillimage" if channel_type == "RADIO" else "zerolatency",
            "-g", os.getenv("HLS_GOP_SIZE", "50") if channel_type == "RADIO" else "60",
            "-keyint_min", os.getenv("HLS_GOP_SIZE", "50") if channel_type == "RADIO" else "60",
