...
def main():
    ...
    playlist = Playlist_Files.select().where(Playlist_Files.day_week == day_week).where(Playlist_Files.render == 1).where(Playlist_Files.time_start >= time_start).order_by(Playlist_Files.time_start.asc()).dicts()
