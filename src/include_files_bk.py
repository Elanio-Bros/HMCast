...
def code_videos(id, value, time_start, time_end, day_week_start, day_week_end, personality_opening=None):
    ...
    if 'opening' in cutoffs.keys():
        if personality_opening != None or id > 0:
            ...
            clip = concatenate_videoclips([clip1, opening, clip2], method='compose')
        else:
            clip = video
    ...
    clip.write_videofile("{}/{}".format(config.TEMP_PATH, file), threads=2)
    ...
    Playlist_Files.insert({"file_id": value['id'], "file": file, "duration": str(timedelta(
